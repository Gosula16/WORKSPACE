...
exports.createJob = async (req, res, next) => {
  try {
    let company = await Company.findById(req.user.companyRef);
    if (!company) {
      // Create a default company if missing
      company = await Company.create({ name: req.user.name + ' Company', description: 'Auto-created company' });
      // Update user with new companyRef
